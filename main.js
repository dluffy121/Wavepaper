...
window.wallpaperPropertyListener = {
	...
	applyUserProperties: function (properties) {
		...
		if (properties.wallpaperchangeinterval) {
			if (properties.wallpaperchangeinterval.value) {
				intervalMinutes = parseFloat(properties.wallpaperchangeinterval.value) * 1000 * 60;
				if (intervalObj != null)
					window.clearInterval(intervalObj);
				intervalObj = window.setInterval(onTimeInterval, intervalMinutes);
			}
		}
		if (properties.changefadeduration) {
			if (properties.changefadeduration.value) {
				fadeTime = parseFloat(properties.changefadeduration.value) * 1000;
				if (fadeTime > intervalMinutes)
					fadeTime = intervalMinutes;
				fadeTime /= 2;
			}
		}
...
function wallpaperAudioListener(audioArray) {
	...
	for (let i = 0; i < audioSample.length / 2; i++) {
		if (maxAudioSample < audioSample[i])
			maxAudioSample = audioSample[i];
	}
}

var maxAudioSample = 0;
...
function run() {
	...
	console.log(wallpaperSettings.fps);
...
function onTimeInterval() {
	...
	currBackgroundColorOpacity = backgroundColorOpacity;
	window.requestAnimationFrame(fadeOverlayIn);
}

var last;
var fpsThreshold = 0;
var intervalMinutes;
var currBackgroundColorOpacity = 0;

function changeWallpaper() {
	window.wallpaperRequestRandomFileForProperty('backgroundimageb', randomImageResponse);
}

function fadeOverlayIn() {

	if (currBackgroundColorOpacity >= 1) {
		currBackgroundColorOpacity = 1;
		window.changeWallpaper();
		last = performance.now();
		window.requestAnimationFrame(fadeOverlayOut);
		return;
	}
	...
	var dt = Math.floor(now - last);

	currBackgroundColorOpacity = dt / fadeTime;
	backgroundColor = "rgba(" + customBgColor + "," + currBackgroundColorOpacity + ")";
}

function fadeOverlayOut() {

	if (currBackgroundColorOpacity <= backgroundColorOpacity) {
		currBackgroundColorOpacity = backgroundColorOpacity;
		return;
	}
	...
	currBackgroundColorOpacity = (1 - dt / fadeTime);
	backgroundColor = "rgba(" + customBgColor + "," + currBackgroundColorOpacity + ")";
}
